...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
    ...
    <body onload="initListGroup('Cog', document.forms[0].Cognitivelevel, document.forms[0].Actionverbs);" style="background-image: none" bgcolor="#FAEBD7">
        ...
        <div id="main">
            ...
            <div id="right-main">
                <h2 class="head">
                    Course <%=course%> -> Module <%=rs5.getString(2)%>: Problem
                </h2>
                <%} catch (Exception ex) {
                                out.println("Unable to connect to Database." + ex.getMessage());
                ...
                <div id="screen">
                    <div class="boxc">
                        <%
                                    ...
                                    int t = 1;
                                    try {
                                        String resultString = "SELECT * FROM moduleproblem WHERE COURSE_ID='" + id + "' AND MODULE_ID=" + moduleid + " ";
                        ...
                        <table border="0" cellpadding="0" class="hbox" cellspacing="0" width="100%" align="center" >
                            <tbody>
                                ...
                                <tr>
                                    <td align="center" width="15%" style=" table-layout: fixed ;border:1px solid #E4E4E4;" rowspan="3" >
                                        <div align="center" style="display: block" id="obj" >
                                            <ol style="font-style: normal">
                                                <%
                                                                                            String[] str1 = rs2.getString(6).split("\\.");
                                    ...
                                    <td align="left" colspan="4"><h3>Module's Problem<%=t++%>.</h3></td>
                                ...
                                <tr>
                                    <td style="border:1px solid #E4E4E4;" align="center">
                                        <%if (rs2.getString(8) != null) {
                                                                                        String ext[] = rs2.getString(8).split("\\.");
                                                                                        String ext1 = ext[3];

                                        %>
                                        <form action="get.jsp" method="get" id="get_<%=t%>">
                                            <input type="hidden" name="extension" value="<%=ext1%>"/>
                                            <input type="hidden" name="filename" value="<%=rs2.getString(8)%>"/>
                                            <a href="javascript:{}" id="solution" name="solution" onclick="document.getElementById('get_<%=t%>').submit();" > Solution </a>
                                        </form>
                                        <%}%>
                                    </td>
                                    <% if (email1.equals("null") && email2.equals("null")) {%>
                                    <form name="courseproblem" action="modulesolution_database.jsp?MPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>" method="POST" enctype="multipart/form-data">
                                    ...
                                    <td style="float: right;" colspan="3">
                                        <a href="JavaScript:window.location='moduleprob_database2.jsp?MPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>'" class="hbox"><img src="Images/images.jpg" alt="" width="20" height="20"/></a>
                                        <a href="JavaScript:window.location='moduleprob_database3.jsp?MPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>'" class="hbox"><img src="Images/downimages.jpg" alt="" width="20" height="20"/></a>
                                        <a href="JavaScript:window.location='module_problem1.jsp?MPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>'" class="hbox">Edit</a>
                                        <a href="JavaScript:window.location='moduleprob_delete.jsp?MPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>&DELETE_FILE=<%=rs2.getString(8)%>'" class="hbox">Delete</a>
                        ...
                        %>
                        <a href="JavaScript:window.location='module_problem1.jsp?MPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>'" class="hbox">Add Comment</a>
                        ...
                        <%
                                                                    ...
                                                                    if (rs2.getString(7) == null || rs2.getString(7).equals("")) {
                                                                        ...
                                                                        out.println("<td align='Justify' style='background-color: #F1F1F1; padding: 5px;'>" + rs2.getString(7) + "</td>");
                                                                    }
                        ...
                        <%
                                    ...
                                    try {
                                        ...
                                        if (email1.equals("null") && email2.equals("null")) {
                                            resultString = "SELECT * FROM moduleproblem WHERE COURSE_ID=" + id + " AND MPROB_ID=" + mprobid + " AND MODULE_ID=" + moduleid + "";
                                        } else {
                                            resultString = "SELECT * FROM moduleproblem WHERE COURSE_ID=" + id + " AND MPROB_ID=" + mprobid + " AND MODULE_ID=" + moduleid + "";
                                        }
                        ...
                        <%}%>
                        <form method="post" action="moduleprob_database1.jsp" name="cog" onsubmit="return submitForm();">
                            <input name="COURSE_ID" value="<%=id%>" type="hidden"/>
                            <input name="MODULE_ID" value="<%=moduleid%>" type="hidden"/>
                            <input name="MPROB_ID" value="<%=mprobid%>" type="hidden"/>
                            ...
                            <table style="border: #777777;border-style: solid; display:none;" width="100%" id="objectivetable">
                                <thead>
                                    <tr class="headline">
                                        <th width="2%"></th>
                                        <th width="5%">S.No.</th>
                                        <th width="93%">Module Objective</th>
                                    </tr>
                                </thead>
                                <%
                                                                            request.setCharacterEncoding("UTF-8");
                                                                            ...
                                                                            int m = 1;
                                                                            try {
                                                                                String resultString1 = "SELECT * FROM moduleobjective where COURSE_ID='" + id + "' AND MODULE_ID='" + moduleid + "'";
