...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
    <head>
        ...
        <script type="text/javascript">
            function check(frm){
                ...
                var PASSWORD_ID=frm.PASSWORD_ID.value;
                var list = document.getElementsByName("Objective");
                //var LOGIN = document.getElementsByName("LOGIN");
                //var PASSWORD_ID = document.getElementsByName("PASSWORD_ID");
                if("cetiitkgp"!=LOGIN && "Pedagogy_cetiitkgp"!=PASSWORD_ID)
                {
                    for (var i = 0; i< list.length; i++) {
                        if(document.cog["Objective"][i].checked){
                            flag ++;
                        }
                    }
                    if (flag == 0) {
            ...
            function show(){
                ...
                if(document.getElementById("objectivetable").style.display=="none")
                {
                    document.getElementById("objectivetable").style.display="block";
            ...
            function value_v(cb){
                ...
                for(var i=0; i < document.form1.Objective.length; i++)
                {
                    if(document.form1.Objective.checked==true)
                    {
                        document.form1.Objective[i].checked = false;
                        if (i == cb) {
                            document.form1.Objective[i].checked= true;
                        }
    ...
    <body onload="initListGroup('Cog', document.forms[0].Cognitivelevel, document.forms[0].Actionverbs);" style="background-image: none" bgcolor="#FAEBD7">
        ...
        <div id="main">
            ...
            <div id="right-main">
                <h2 class="head">
                    Course <%=course%> -> Module <%=modulename%> -> Unit <%=rs5.getString(2)%>: Problem
                </h2>
                <%} catch (Exception ex) {
                                out.println("Unable to connect to Database." + ex.getMessage());
                ...
                <div id="screen">
                    <div class="boxc">
                        <%
                                    ...
                                    String uprobid = request.getParameter("UPROB_ID");
                                    request.setCharacterEncoding("UTF-8");
                                    ...
                                    int t = 1;
                                    try {
                                        String resultString = "SELECT * FROM unitproblem WHERE COURSE_ID='" + id + "' AND MODULE_ID=" + moduleid + " AND UNIT_ID=" + unitid + "";
                                        pstatement2 = connection2.prepareStatement(resultString);
                                        rs2 = pstatement2.executeQuery();
                                        while (rs2.next()) {%>
                        <table border="0" cellpadding="0" class="hbox" cellspacing="0" width="100%" align="center" >
                            <tbody>

                                <tr>
                                    <td align="center" width="15%" style=" table-layout: fixed ;border:1px solid #E4E4E4;" rowspan="3" >
                                        <div align="center" style="display: block" id="obj" >
                                            <ol style="font-style: normal">
                                                <%
                                                                                            String[] str1 = rs2.getString(7).split("\\.");
                                                                                            for (int d = 0; d < str1.length; d++) {
                                                %>

                                                <li style="text-decoration: underline;font-weight: bold"><%=str1[d]%></li>
                                                <%}%>
                                            </ol>
                                    ...
                                    <td align="left" colspan="4"><h3>Problem <%=t++%>.</h3></td>

                                </tr>
                                <tr><td  class="timestamp" colspan="4">Last updated at <img width="16" height="16" title="Date and Time" src="Images/clock-white-bg.png" alt="Date and Time"/> <%=rs2.getString(3)%>  </td></tr>
                                <tr>
                                    <td colspan="4" align="Justify" width="100%" style="table-layout: fixed"> <%=rs2.getString(2)%></td>
                                </tr>

                                <tr>
                                    <td style="border:1px solid #E4E4E4;" align="center">
                                        <%if (rs2.getString(9) != null) {
                                                                                        String ext[] = rs2.getString(9).split("\\.");
                                                                                        String ext1 = ext[4];

                                        %>
                                        <form action="get.jsp" method="get" id="get_<%=t%>">
                                            <input type="hidden" name="extension" value="<%=ext1%>"/>
                                            <input type="hidden" name="filename" value="<%=rs2.getString(9)%>"/>
                                            <a href="javascript:{}" id="solution" name="solution" onclick="document.getElementById('get_<%=t%>').submit();" > Solution </a>
                                        </form>
                                        <%}%>
                                    </td>
                                    <% if (email1.equals("null") && email2.equals("null")) {%>
                                    <form name="courseproblem" action="unitsolution_database.jsp?UPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&UNIT_ID=<%=unitid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>" method="POST" enctype="multipart/form-data">
                                        <td style="float: left;display: block" colspan="1" id="doc2<%=t%>" height="30px">
                                            <b>Add Solution</b><input name="file<%=t%>" id="file<%=t%>" type="file" style="border:2px solid #777777 ;  "></input>
                                            <input type="submit" value="Submit"  name="submit" onclick= "return validateForm('file<%=t%>')"/>
                                    ...
                                    <td style="float: right;" colspan="3">
                                        ...
                                        <a href="JavaScript:window.location='unitprob_database2.jsp?UPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&UNIT_ID=<%=unitid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>'" class="hbox"><img src="Images/images.jpg" alt="" width="20" height="20"/></a>
                                        <a href="JavaScript:window.location='unitprob_database3.jsp?UPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&UNIT_ID=<%=unitid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>'" class="hbox"><img src="Images/downimages.jpg" alt="" width="20" height="20"/></a>
                                        <a href="JavaScript:window.location='unit_problem1.jsp?UPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&UNIT_ID=<%=unitid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>'" class="hbox">Edit</a>
                                        <a href="JavaScript:window.location='unitprob_delete.jsp?UPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&UNIT_ID=<%=unitid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>&DELETE_FILE=<%=rs2.getString(9)%>'" class="hbox">Delete</a>
                                    </td>
                                    <%}%>
                                </tr>
                        ...
                        <%
                                                                    ...
                                                                    out.println("</tr>");
                                                                    out.println("<tr><td  class='timestamp'>Last updated at <img width='16' height='16' title='Date and Time' src='Images/clock-white-bg.png' alt='Date and Time'>" + rs2.getString(3) + "</td></tr>");
                                                                    out.println("<tr>");
                        ...
                        %>
                        <a href="JavaScript:window.location='unit_problem1.jsp?UPROB_ID=<%=rs2.getString(1)%>&COURSE_ID=<%=id%>&MODULE_ID=<%=moduleid%>&UNIT_ID=<%=unitid%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>'" class="hbox">Add Comment</a>
                        <%
                                                                    }
                                                                    out.println("</td>");
                                                                    out.println("</tr>");
                        ...
                        <%
                                    ...
                                    try {
                                        ...
                                        if (email1.equals("null") && email2.equals("null")) {
                                            resultString = "SELECT * FROM unitproblem WHERE COURSE_ID='" + id + "' AND MODULE_ID=" + moduleid + " AND UNIT_ID=" + unitid + " AND UPROB_ID=" + uprobid + "";
                                        } else {
                                            resultString = "SELECT * FROM unitproblem WHERE COURSE_ID='" + id + "' AND MODULE_ID=" + moduleid + " AND UNIT_ID=" + unitid + " AND UPROB_ID=" + uprobid + "";
                                        }
                            ...
                            <tbody>
                                <tr>
                                    <td align="center" valign="middle" >
                                        ...
                                        <div id="err-container2" class="fk-err-all" style="display: none;" align="center"  >

                                            <p>* Please select Objective for This Problem</p>
                        ...
                        <%
                                                                    if (email1.equals("null")) {
                        %>
                        <h2 align="center">Define your problem here:-</h2>
                        <%} else {%>
                        <h2 align="center">Add Comment:-</h2>

                        <%}%>
                        <form method="post" action="unitprob_database1.jsp" name="cog" onsubmit="return submitForm();">
                            ...
                            <input name="UPROB_ID" value="<%=uprobid%>" type="hidden"/>
                            ...
                            <input name="PASSWORD_ID" id="PASSWORD_ID" value="<%=password_id%>" type="hidden"/>
                            <table align="center">
                                <tbody>
                                    <tr>
                                        <td>
                                    ...
                                    <tr>
                                        <td align="right" >
                                            <a href="JavaScript:void(0);" id="test" style="background-color:darkgray;color:#0000ff" onclick="show()">Link With Objective</a>
                            ...
                            <table style="border: #777777;border-style: solid; display:none;" width="100%" id="objectivetable">
                                <thead>
                                    <tr class="headline">
                                        <th width="2%"></th>
                                        <th width="5%">S.No.</th>
                                        <th width="93%">Unit Objective:-</th>
                                    </tr>
                                </thead>
                                <%
                                                                            request.setCharacterEncoding("UTF-8");
                                                                            String connectionURL3 = "jdbc:mysql://localhost:3306/pedagogy_new?useUnicode=true&characterEncoding=UTF-8";
                                                                            Connection connection3 = null;
                                                                            PreparedStatement pstatement3 = null;
                                                                            ResultSet rs3 = null;
                                                                            Class.forName("com.mysql.jdbc.Driver").newInstance();
                                                                            connection3 = DriverManager.getConnection(connectionURL3, "root", "Pedagogy_cet123");
                                                                            int m = 1;
                                                                            try {
                                                                                String resultString1 = "SELECT * FROM unitobjective where COURSE_ID='" + id + "' AND MODULE_ID='" + moduleid + "' AND UNIT_ID='" + unitid + "'";
                                                                                pstatement3 = connection3.prepareStatement(resultString1);
                                                                                rs3 = pstatement3.executeQuery();
                                                                                rs3.last();
                                                                                int p = rs3.getRow();
                                                                                request.getSession().setAttribute("regis111", p);
                                                                                rs3.beforeFirst();
                                                                                while (rs3.next()) {
                                %>
                                ...
                                <tbody>
                                    <tr>
                                        <td align="center"><input align="middle" type="checkbox"  name="Objective" id="Objective<%=m%>" value="Objective<%=m%>." /></td>
                                        <td align="center" valign="middle"><b><%=m++%>.</b></td>
                                        <td align="justify"><b><%=rs3.getString(4)%></b></td>
                                    </tr>
                                    <tr >
                                        <td height="10px"></td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td></td>
                                        <td></td>
                                        <td align="right"><input align="right" type="submit" value="Close" onclick="return hide()" /> </td>
                                    </tr>
                                </tbody>
                            </table>
                        </form>
