    ...
    <body>
        ...
        <div id="main" style="min-height:800px">
            <div id="left" style="width:100%;">
                <div id="flash"></div>
                <h2 class="head">Final Submission </h2>
                <div id="screen" style="min-height: 800px" align="center">
                    ...
                    <%
                                ...
                                if ("Confirm".equals(stat)) {
                                    ...
                                    try {
                                        ...
                                        String queryString = "UPDATE register_user SET CNAME = 'Completed' WHERE USER_ID='" + user_id + "'";

                                        pstatement1 = connection1.prepareStatement(queryString);
                                        //pstatement1.setString(1, status);
                                        pstatement1.executeUpdate();

                                        String val = "SELECT FINAL_SUBMISSION_TIME FROM course WHERE COURSE_ID=" + course_id + "";
                                        pstatement4 = connection1.prepareStatement(val);
                                        rs4 = pstatement4.executeQuery();
                                        while (rs4.next()) {
                                            if (rs4.getString(1).equals("0000-00-00 00:00:00")) {
                                                String queryString4 = "UPDATE course SET FINAL_SUBMISSION_TIME=DATE_FORMAT(NOW(),'%d %b %y %h:%i %p') WHERE COURSE_ID=" + course_id + "";
                                                pstatement3 = connection1.prepareStatement(queryString4);

                                            } else {

                                                String queryString4 = "UPDATE course SET FINAL_SUBMISSION_TIME=CONCAT('" + rs4.getString(1) + "',',',DATE_FORMAT(NOW(),'%d %b %y %h:%i %p')) WHERE COURSE_ID=" + course_id + "";
                                                pstatement3 = connection1.prepareStatement(queryString4);
                                            }
                    ...
                    <h3> <a href="main_page.jsp?LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>" style="font-size: large; font-weight: bold"> Back</a></h3>
                    <%} else {%>
                    <h3 align="center" style="color: #9F1D20"> Thank You for Developing the Course. Do you want to release the Course? </h3>
                    <h4 align="center" >If you press Confirm button, then you can not modify/update your course and your course will be released. Your user name and password will be deactivated. </h4>
                    ...
                    <form action="submission.jsp" method="post">
                        ...
                        <input type="submit" name="stat" value="Confirm" />
                    </form>
                    <a href="coursename.jsp?LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>&COURSE_ID=<%=id%>&email1=<%=email1%>&email2=<%=email2%>" style="font-size: large; font-weight: bold"> Back</a>
                    <%}%>
                </div>
            </div>
