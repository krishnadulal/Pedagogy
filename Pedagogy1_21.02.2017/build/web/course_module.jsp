<%--
    Document   : index
    Created on : 4 Jun, 2013, 10:27:16 AM
    Author     : Krishna
--%>
            ...
            <div id="right-main">
                ...
                <div id="screen">
                    ...
                    <div id="pras" class="hbox">
                        ...
                        <%
                            if (email1.equals("null") && email2.equals("null")) {%>
                        <form name="modulename_change_<%=p%>" id="modulename_change_<%=p%>" action="modulename_change.jsp" method="get" >
                            ...
                            <a href="javascript:{}" id="d_<%=p%>" name="d_<%=p%>" style="color: #0054ff ;text-decoration: underline;font-size: medium;float: right;display:none;" onclick="document.getElementById('modulename_change_<%=p%>').submit();">Save</a>
                            <input type="hidden" name="COURSE_ID" value=" <%=id%>"/>
                            <input type="hidden" name="MODULE_CHANGE" value="b_<%=p%>"/>
                            <input type="hidden" name="MODULE_ID" value=" <%=rs4.getString(1)%>"/>
                        ...
                        <br></br>
                        <a href="JavaScript:window.location='del.jsp?MODULE_ID=<%=rs4.getString(1)%>&COURSE_ID=<%=id%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>'" name="delete" style="text-decoration: underline;float: right;" > Delete </a>
                        <%} else {%>
                        <br></br>
                        <%}%>
                        <div class="hboxd">
                        ...
                        <div class="hboxc">
                            <ul>
                                <ol>
                                    <li>
                                        <a href="module_overview.jsp?COURSE_ID=<%=id%>&MODULE_ID=<%=rs4.getString(1)%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>">Module Overview </a>
                                    </li>
                                    <li>
                                        <a href="module_objectives.jsp?MODULE_ID=<%=rs4.getString(1)%>&COURSE_ID=<%=id%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>">Module Objectives</a>
                                    </li>
                                    <li>
                                        <a href="module_unit.jsp?MODULE_ID=<%=rs4.getString(1)%>&COURSE_ID=<%=id%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>">Module Units</a>
                                    </li>
                                    <li>
                                        <a href="module_problems.jsp?MODULE_ID=<%=rs4.getString(1)%>&COURSE_ID=<%=id%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>">Module Level Problems</a>
                                    </li>
                                    <li>
                                        <a href="module_strategy.jsp?MODULE_ID=<%=rs4.getString(1)%>&COURSE_ID=<%=id%>&email=<%=email%>&email1=<%=email1%>&email2=<%=email2%>&LOGIN=<%=login%>&PASSWORD_ID=<%=password_id%>">Module's Learning Strategy</a>
                                    </li>
                    ...
                    <div align="center" id="head">
                        <h1 style="color: #BB0000; ">Add New Module:-</h1>
                    </div>
                    <div id="check">
                    ...
                    <div class="boxc">
                        ...
                        <form name="modulename" action="module.jsp" method="get" >
                            ...
                            <table border="0" align="center" cellpadding="0" cellspacing="0" >
                                ...
                                <tbody>
                                    <tr>
                                        <input type="hidden" name="COURSE_ID" value=" <%=id%>"/>
                                        <input align="middle" value="<%=email%>" type="hidden" name="email" />
                                        <input align="middle" value="<%=email1%>" type="hidden" name="email1" />
                                        <input align="middle" value="<%=email2%>" type="hidden" name="email2" />
                                        <input name="LOGIN" value="<%=login%>" type="hidden"/>
                                        <input name="PASSWORD_ID" value="<%=password_id%>" type="hidden"/>
                                        <td>

                                        </td>
                                        <td  align="left" style="font-size: large;font-weight: bold">Give Module Name:-</td>
                                        <td  align="left"><input align="middle"  value="" type="text" name="module" id="module" style="width:400px; height:30px" /> </td>
                                        <td width="30px"></td>
